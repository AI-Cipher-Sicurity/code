...
def final_select_top_k_keys(cts, final_keys, net,k):
    ...
    Z = Z.reshape(-1, 1024)
    s = np.sum(Z, axis=1)
    top_three_indices = np.argsort(s)[-k:][::-1]  # 反转以得到最大的k个索引
...
def process_array(a, b):
    ...
    for c in a:
        ...
        # 将十进制数 c 转换为二进制字符串
        binary_str = format(c, '048b')

        # 反转 b 中指定的一个比特
        for bit in b:
            flipped_str = binary_str[:bit] + ('0' if binary_str[bit] == '1' else '1') + binary_str[bit + 1:]
        ...
        #         flipped_str[b[i]] = '0' if binary_str[b[i]] == '1' else '1'
        #         flipped_str[b[j]] = '0' if binary_str[b[j]] == '1' else '1'
        #         new.append(int(''.join(flipped_str), 2))

    return new


def find_differing_bits(a, b):
    a = int(a)
    b = int(b)
    print(type(a))
    print(type(b))
    ...
    # 使用异或操作找到所有不同的位
    xor_result = a ^ b
    print('异或的结果是:',xor_result)
    # 检查每一位是否不同
    for i in range(48):
        if xor_result & (1 << i):
            differing_bits.append(i)
    print("不同的位有：", differing_bits)
...
def attack_with_dual_NDs(t, nr, diffs, NBs, nds, bits, c, k):
    ...
    for i in range(t):
        ...
        imp_ct0, imp_ct1, imp_ct2, imp_ct3 =[],[],[],[]
        ...
        while True:
            ...
            if len(sur_kg_4) == 0:
                ...
                break
        if num == -1:
            with open('outputnew11.txt', 'a') as f:
                print(' ')
                print('this trial fails.')
                print('{} plaintext structures are generated.'.format(data_num))
                print('the time consumption is ', time.time() - start)
        ...
        resultkey = final_select_top_k_keys([selected_ct0, selected_ct1, selected_ct2, selected_ct3],final_keys,nets[4],k=1)
        end = time.time()
...
if __name__ == '__main__':
    nd1 = './trained_net/model_7r_diff55_acc68_depth1_num_epochs5.h5'
    nd2 = './trained_net/model_7r_diff67_acc60_depth1_num_epochs5.h5'
    nd3 = './trained_net/model_7r_diff77_acc59_depth1_num_epochs5.h5'
    nd4 = './trained_net/model_7r_diff89_acc60_depth1_num_epochs5.h5'
    nd5 = './trained_net/model_7r_diff55all_acc83_depth1_num_epochs5.h5'  # 最终的筛选器
