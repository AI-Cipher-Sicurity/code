...
# #   # else:
# #   #     bitall = np.concatenate((bit, [i + 32 for i in bit], [(i+2)%32+64for i in bit],[i+64+32 for i in bit[3:]]))
# #   # #     resultbitall = np.copy(bitall)
# #   # #     for i in range(1, pair):
# #   # #         bitall_plus_n = bitall + 128 * i
# #   # #         resultbitall = np.concatenate((resultbitall, bitall_plus_n))
...
# #   #       ctdata0l2, ctdata0r2 = dec_one_round_for_random((ctdata0l, ctdata0r), ks_use_back)
# #   #       ctdata1l2, ctdata1r2 = dec_one_round_for_random((ctdata1l, ctdata1r), ks_use_back)
# #   #       ctdata0l2 = np.array(ctdata0l2).flatten()
# #   #       ctdata1l2 = np.array(ctdata1l2).flatten()
# #   #       # 这里表示是使用连接
# #   #       if isct:
# #   #           X += [ctdata0l,ctdata0r,ctdata1l,ctdata1r,ctdata0l2,ctdata0r2,ctdata1l2,ctdata1r2]
# #   #       else:
# #   #           # 这里这里翻回来了
# #   #           X = [ctdata0l^ctdata1l,ctdata0r^ctdata1r,ctdata0r2^ctdata1r2,ctdata0l2^ctdata1l2]
# #   #           X = convert_to_binary(X)
# #   #           X = X[:, bitall]
# #   #           # 填充好
# #   #           padded_X = np.pad(X, ((0, 0), (0, 3)), 'constant')
# #   #           if i == 0:
# #   #               X_bin = padded_X
# #   #           else:
# #   #               X_bin = np.concatenate((X_bin, padded_X), axis=1)
# #   Y = np.frombuffer(urandom(n), dtype=np.uint8);
# #   Y = Y & 1;
# #   Y1 = np.tile(Y, pair);
...
#
#     # 从32位的二进制数中提取第2到第12位
#     extracted_bits = binary_repr[1:12]
#
#     # 将提取的位序列转为十进制数并返回
...
#   # 取出指定位置的比特
#   vfunc = np.vectorize(extract_bits)
#   last_key_extract = vfunc(last_key)
#   last_key_extract = last_key_extract << 20
#   ctdata0l, ctdata0r = encrypt((plain0l, plain0r), ks);
#
#   ctdata1l, ctdata1r = encrypt((plain1l, plain1r), ks);
